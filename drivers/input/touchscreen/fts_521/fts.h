...
#define FTS_USE_BACKLIGHT_NOTIFIER 0
...
#define FW_H_FILE
/*
 * Keep the first AGNOS bring-up boot-safe. The Lineage driver normally runs a
 * firmware/flash update from probe, which can block later initcalls when built
 * into the kernel. Firmware update remains available through sysfs.
 */
#define FTS_BOOT_SAFE_NO_AUTO_FW_UPDATE
