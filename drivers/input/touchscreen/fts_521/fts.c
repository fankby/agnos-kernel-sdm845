...
/*
 * AGNOS' 4.9 SDM845 tree does not carry Lineage's downstream
 * drm_notifier API. Touch input does not need that notifier for the first
 * bring-up pass, so keep suspend/resume display hooks disabled locally.
 */
#undef CONFIG_DRM
#ifdef CONFIG_DRM
...
bool fts_fod_status;
#endif
static inline void lpm_disable_for_input(bool on)
{
}
...
static inline const char *dsi_get_display_name(void)
{
	return NULL;
}
...
#ifdef FW_H_FILE
	if (fw_name && info->board->default_fw_name &&
	    !strcmp(fw_name, info->board->default_fw_name))
		fw_name = "NULL";
#endif
...
#else
#ifdef FTS_BOOT_SAFE_NO_AUTO_FW_UPDATE
	pr_info("Boot-safe mode: skip automatic FW update; initialize sensing only.\n");
	error = fts_init_sensing(info);
	if (error < OK) {
		pr_err("Cannot initialize sensing in boot-safe mode ERROR %08X\n",
			error);
		error = OK;
	}
#else
...
#ifndef FW_UPDATE_ON_PROBE
#ifndef FTS_BOOT_SAFE_NO_AUTO_FW_UPDATE
...
static int fts_remove(struct spi_device *client)
{
	...
	destroy_workqueue(info->event_wq);
#if !defined(FW_UPDATE_ON_PROBE) && !defined(FTS_BOOT_SAFE_NO_AUTO_FW_UPDATE)
